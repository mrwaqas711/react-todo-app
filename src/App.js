...
const App = () => {
  ...
  const bioData = [
    { id: 1, name: 'Elvis', age: 27, contact: 12345678, address: 'House #3', isSelected: false },
    { id: 2, name: 'Quin', age: 25, contact: 12233243438, address: 'Street #5', isSelected: false },
    { id: 3, name: 'Liu', age: 29, contact: 20192721, address: 'Wuchang liu hao #5', isSelected: false }
  ];
  ...
  const handleCheck = (checked, idx) => {
    users[idx].isSelected = checked;
    setUsers([...users]);
  };
  const handleDelete = () => {
    const newList = users.filter((x) => !x.isSelected);
    setUsers(newList);
  };
  return (
    <MainContainer>
            ...
            <div key={idx} className='body-set'>
              {
                !selected ?
                  ...
                  <p className='btn-checked'>
                    <span>
                      <input type='checkbox' name={user.name} checked={user.isSelected} onChange={(e) => handleCheck(e.target.checked, idx)} />
                    </span>
                    <span>
                      <button className='edit-btn' onClick={() => handleEdit(idx)}>
                        <span className='dell-icon'><img src='/edit.svg' alt='' width='15' height='15' /></span>
